...
ACTIVE_BOT_COMMANDS = (
    ...
    ("apps_status", "active apps status and statistics"),
    ("summary", "today's leads summary"),
    ("analytics", "saved listing view analytics"),
    ...
    ("deploy", "start a production deploy and show its progress"),
)
...
def _build_bot_commands() -> list[BotCommand]:
    return [BotCommand(command=command, description=_(description)) for command, description in ACTIVE_BOT_COMMANDS]
...
def build_help_message() -> str:
    lines = [
        ...
        "",
        _("The bot sends new Gmail leads, provides quick status buttons, and links to the mobile admin."),
        _("It also shows service health, unread leads, and a short operational summary."),
        "",
        _("⚡ <b>Active commands</b>"),
    ]
    lines.extend(f"/{command} — {html.escape(_(description))}" for command, description in ACTIVE_BOT_COMMANDS)
    lines.extend(["", _("🔘 <b>Alert buttons</b>"), _("Take to work · New · Ignore · Status · Open Mobile")])
    return "\n".join(lines)
...
async def handle_help_command(update, context):
    ...
    if not is_allowed_update(update):
        ...
        return
    await update.effective_message.reply_text(build_help_message(), parse_mode="HTML", disable_web_page_preview=True)
