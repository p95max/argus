...
def publication_listing_state(alert: MarketplaceAlert) -> str:
    """Describe whether a publication notice can create a tracked listing."""
    if not _looks_like_publication_notice(alert):
        return "not_publication"
...
def publication_listing_candidate(alert: MarketplaceAlert):
    ...
    if publication_listing_state(alert) != "available":
        return None

    listing_url = _extract_listing_url(alert) or _url_from_listing_id(alert)
    if not listing_url:
        return None
    return validate_kleinanzeigen_url(listing_url)
